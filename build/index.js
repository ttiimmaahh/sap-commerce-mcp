...
async function makeRequest(url, token, method = 'GET', body) {
    const headers = {
...
function formatCartEntry(entry, index) {
    const sections = [
        `${index + 1}. **${entry.product?.name || 'Product'}** (${entry.product?.code || 'N/A'})`,
        `   Quantity: ${entry.quantity}`,
...
server.tool("product-search", "Search for products in SAP Commerce", {
    ...
    const token = extra._meta?.sapToken;
    if (!token) {
        return {
            content: [
                {
    ...
    try {
        ...
        const productSearchData = await makeRequest(searchUrl, token);
...
server.tool("get-base-sites", "Get available base sites from SAP Commerce", {
    fields: z.string().optional().default("DEFAULT").describe("Response field configuration"),
}, async ({ fields }, extra) => {
    const token = extra._meta?.sapToken;
    if (!token) {
    ...
    try {
        ...
        const baseSitesData = await makeRequest(baseSitesUrl, token);
...
server.tool("order-history", "Get user's order history from SAP Commerce (limited to 5 most recent orders)", {
    baseSiteId: z.string().describe("Base site identifier (e.g., 'electronics-spa')"),
    ...
    try {
        ...
        const orderHistoryData = await makeRequest(orderHistoryUrl, token);
        if (!orderHistoryData) {
            return {
                content: [
                    {
...
server.tool("order-details", "Get detailed information about a specific order from SAP Commerce.  The user will provide the order code, also known as order number or order id.", {
    baseSiteId: z.string().describe("Base site identifier (e.g., 'electronics-spa')"),
    userId: z.string().describe("User identifier (use 'current' for authenticated user)"),
    ...
    try {
        ...
        const orderData = await makeRequest(orderDetailsUrl, token);
        if (!orderData) {
            return {
                content: [
                    {
...
server.tool("add-to-cart", "Add a product to the user's shopping cart in SAP Commerce", {
    baseSiteId: z.string().describe("Base site identifier (e.g., 'electronics-spa')"),
    userId: z.string().describe("User identifier (use 'current' for authenticated user)"),
    ...
    try {
        ...
        const cartModificationData = await makeRequest(fullUrl, token, 'POST', requestBody);
...
server.tool("get-cart", "Get the current shopping cart details from SAP Commerce", {
    baseSiteId: z.string().describe("Base site identifier (e.g., 'electronics-spa')"),
    userId: z.string().describe("User identifier (use 'current' for authenticated user)"),
    ...
    try {
        ...
        const cartData = await makeRequest(cartUrl, token);
        if (!cartData) {
            return {
                content: [
                    {
...
server.tool("update-cart-entry", "Update the quantity of a product in the user's cart or remove it entirely", {
    baseSiteId: z.string().describe("Base site identifier (e.g., 'electronics-spa')"),
    userId: z.string().describe("User identifier (use 'current' for authenticated user)"),
    ...
    try {
        ...
        const cartModificationData = await makeRequest(fullUrl, token, method, quantity > 0 ? requestBody : undefined);
        if (!cartModificationData) {
            return {
                content: [
                    {
...
server.tool("set-delivery-address", "Set the delivery address for the user's cart", {
    baseSiteId: z.string().describe("Base site identifier (e.g., 'electronics-spa')"),
    userId: z.string().describe("User identifier (use 'current' for authenticated user)"),
    ...
    try {
        ...
        if (addressId) {
            ...
            if (regionIsocode)
                requestBody.region = { isocode: regionIsocode };
            if (phone)
                requestBody.phone = phone;
        }
        const addressData = await makeRequest(setAddressUrl, token, 'PUT', requestBody);
        if (!addressData) {
            return {
                content: [
                    {
...
server.tool("set-delivery-mode", "Set the delivery mode for the user's cart", {
    baseSiteId: z.string().describe("Base site identifier (e.g., 'electronics-spa')"),
    userId: z.string().describe("User identifier (use 'current' for authenticated user)"),
    ...
    try {
        ...
        const deliveryModeData = await makeRequest(setDeliveryModeUrl, token, 'PUT');
...
server.tool("get-delivery-modes", "Get available delivery modes for the user's cart", {
    baseSiteId: z.string().describe("Base site identifier (e.g., 'electronics-spa')"),
    userId: z.string().describe("User identifier (use 'current' for authenticated user)"),
    cartId: z.string().optional().default("current").describe("Cart identifier (default: 'current')"),
    ...
    try {
        ...
        const deliveryModesData = await makeRequest(deliveryModesUrl, token);
...
server.tool("place-order", "Place an order from the user's current cart", {
    baseSiteId: z.string().describe("Base site identifier (e.g., 'electronics-spa')"),
    userId: z.string().describe("User identifier (use 'current' for authenticated user)"),
    cartId: z.string().optional().default("current").describe("Cart identifier (default: 'current')"),
    ...
    try {
        ...
        const orderData = await makeRequest(fullUrl, token, 'POST', requestBody);
        if (!orderData) {
            return {
                content: [
                    {
...
server.tool("b2b-add-to-cart", "Add a product to an organization user's B2B cart in SAP Commerce", {
    ...
    try {
        ...
        const cartEntry = await makeRequest(fullUrl, token, 'POST');
        if (!cartEntry) {
            return {
                content: [
                    {
...
server.tool("b2b-get-cart", "Get organization user's current B2B cart details from SAP Commerce", {
    baseSiteId: z.string().describe("Base site identifier (e.g., 'powertools-spa')"),
    ...
    try {
        ...
        console.log("fullUrl: ", fullUrl);
        const cartData = await makeRequest(fullUrl, token);
        if (!cartData) {
            return {
                content: [
                    {
...
server.tool("b2b-update-cart-entry", "Update the quantity of a product in an organization user's B2B cart or remove it entirely", {
    baseSiteId: z.string().describe("Base site identifier (e.g., 'powertools-spa')"),
    orgUserId: z.string().describe("Organization user identifier (e.g., 'mark.rivers@pronto-hw.com')"),
    entryNumber: z.number().describe("Cart entry number to update"),
    quantity: z.number().describe("New quantity (use 0 to remove the item)"),
    ...
    try {
        ...
        if (quantity === 0) {
            ...
            const updatedEntry = await makeRequest(fullUrl, token, 'PUT', requestBody);
            if (!updatedEntry) {
                return {
                    content: [
                        {
...
server.tool("b2b-place-order", "Place an order from an organization user's B2B cart in SAP Commerce", {
    baseSiteId: z.string().describe("Base site identifier (e.g., 'powertools-spa')"),
    orgUserId: z.string().describe("Organization user identifier (e.g., 'mark.rivers@pronto-hw.com')"),
    cartId: z.string().optional().default("current").describe("Cart identifier (default: 'current')"),
    ...
    try {
        ...
        const fullUrl = `${url}?${params.toString()}`;
        console.log("fullUrl: ", fullUrl);
        const orderData = await makeRequest(fullUrl, token, 'POST');
        if (!orderData) {
            return {
                content: [
                    {
